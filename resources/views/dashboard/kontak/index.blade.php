...
Total Pesan: <button type="button" class="btn btn-outline-secondary">{{ $total }}</button><br>
            ...
            @if ($message = Session::get('success'))
                ...
                <div class="alert mt-3 alert-success d-flex align-items-center" role="alert" id="alert">
                    {{-- icon --}}
            ...
            <tr class="@if (isset($contact->is_read)) @else bg-gradient-secondary @endif">
                ...
                <td>
                    <form action="/dashboard/pesanmasuk/isread/{{ $contact->id }}" method="post" class="d-inline">
                        ...
                        <button class="badge" type="submit"
                            style="background-color:rgb(68, 233, 164); color:black; ;border: 0">Save</button>
                    </form>
                ...
                <td>
                    <form action="/dashboard/pesanmasuk/{{ $contact->id }}" method="post">
                        ...
                        <button type="submit" class="badge bg-danger border-0 text-white"
                            onclick="return confirm('Yakin ingin menghapus Kontak ini?')">Delete</button>
                    </form>
